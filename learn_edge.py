...
parser.add_argument('--use_contrastive', action='store_true', help='add T-HyCroD multi-view contrastive loss during link prediction')
...
if args.model_name == 'tgat':
    ...
    MODEL_SAVE_PATH = f'./saved_models/{args.prefix}-{args.model_name}-{args.agg_method}-{args.attn_mode}-{args.data}.pth'
    get_checkpoint_path = lambda epoch: f'./saved_checkpoints/{args.prefix}-{args.model_name}-{args.agg_method}-{args.attn_mode}-{args.data}-{epoch}.pth'
...
for epoch in range(NUM_EPOCH):
    ...
    for k in range(num_batch):
        ...
        if MODEL == 'thycrod' and args.use_contrastive:
            pos_prob, neg_prob, loss_dict = tgan.contrast(
                src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS, return_loss=True
            )
        else:
            pos_prob, neg_prob = tgan.contrast(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS)
            loss_dict = {}
        ...
        link_loss = loss
        if loss_dict:
            loss = loss + loss_dict['loss_contrast']
    ...
    if MODEL == 'thycrod' and args.use_contrastive and loss_dict:
        logger.info(tgan.format_loss_log(link_loss.detach(), loss_dict))
    ...
    if early_stopper.early_stop_check(val_ap):
        ...
        tgan.load_state_dict(torch.load(best_model_path, map_location=device, weights_only=False))
...
logger.info('Saving {} model'.format('TGAN' if MODEL == 'tgat' else 'T-HyCroD'))
torch.save(tgan.state_dict(), MODEL_SAVE_PATH)
logger.info('{} model saved'.format('TGAN' if MODEL == 'tgat' else 'T-HyCroD'))
